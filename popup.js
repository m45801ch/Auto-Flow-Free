...
const i18n = {
  "zh-TW": {
    ...
    voiceDefault: "未設定語音",
    voicePrefix: "旁白語音：",
    voiceDefaultSuffix: "（預設）",
    voiceAutoVeo: "依 Veo 自動生成（不套用語音）",
    voiceSegOff: "本段不套用語音（依 Veo 自動生成）",
    voiceNone: "無（不套用語音）",
    btnVoicePreview: "試聽",
  ...
  en: {
    ...
    voicePrefix: "Narration voice: ",
    voiceDefaultSuffix: " (default)",
    voiceAutoVeo: "Auto-generated by Veo (no voice applied)",
    voiceSegOff: "No voice applied for this segment (auto by Veo)",
    voiceNone: "None (no voice applied)",
  ...
  "zh-CN": {
    ...
    voiceDefault: "未配置语音",
    voicePrefix: "旁白语音：",
    voiceDefaultSuffix: "（默认）",
    voiceAutoVeo: "由 Veo 自动生成（不应用语音）",
    voiceSegOff: "本段不应用语音（由 Veo 自动生成）",
    voiceNone: "无（不应用语音）",
    btnVoicePreview: "试听",
...
function renderPreview() {
  ...
  sessionFrames.forEach(fr => {
    ...
    cell.querySelector(".cell-status").style.color = "var(--success)";
  });
...
function rebuildVoiceOptions() {
  ...
  select.innerHTML = '<option value="">' + t("voiceNone") + '</option>';
  VOICES.forEach(v => {
    const opt = document.createElement("option");
    opt.value = v.name;
...
function bindUI() {
  ...
  // Frames-to-video: frame handling option (first / firstLast / all)
  const frameOptSel = document.getElementById("frameOption");
  if (frameOptSel) {
    frameOptSel.value = settings.frameOption || "first";
    frameOptSel.addEventListener("change", () => {
      settings.frameOption = frameOptSel.value;
      saveSettings();
    });
  }
  ...
  charToggle.addEventListener("change", () => {
    ...
    saveSettings();
    updateCharScanState();
    if (settings.charEnabled) doAutoCharScan();
  });
  ...
  if (voiceSelect) {
    ...
    voiceSelect.addEventListener("change", () => { settings.defaultVoice = voiceSelect.value; saveSettings(); updatePerPromptDurList(); });
  }
...
function updatePerPromptDurList() {
  ...
  list.innerHTML = "";
  // Pool of character names for per-segment matching: user-checked names take
  // priority, falling back to all scanned names (same rule as the content script).
  const charPool = ((settings.charSelected && settings.charSelected.length > 0)
    ? settings.charSelected : (settings.charNames || [])).slice();
  // Current mode's voice support (text2video / components2video / agent)
  const supportsVoice = settings.mode === "text2video" ||
    settings.mode === "components2video" || settings.mode === "agent";
  prompts.forEach((text, i) => {
    ...
    head.className = "dur-head";
    head.innerHTML = '<div class="dur-num">#' + (i + 1) + '</div>' +
      '<div class="dur-text"><b>' + escapeHtml(preview) + '</b> <span class="dur-chars">' + text.length + " chars · " + getPerPromptDuration(i) + t("perDurSec") + "</span></div>";
    head.appendChild(sel);
    row.appendChild(head);
    // Meta row: voice + matched character (thumbnail), mirrors the content-script
    // matching (token-aware substring + token-fragment match).
    const meta = document.createElement("div");
    meta.className = "dur-meta";
    const hits = popupCharsInText(text, charPool);
    if (supportsVoice) {
      const voiceSel = popupVoiceForPrompt(text);
      const voiceNote = document.createElement("div");
      voiceNote.className = "dur-voice";
      let voiceText = "";
      let voiceOff = false;
      if (voiceSel === "__NOVOICE__") {
        voiceText = t("voicePrefix") + " " + t("voiceSegOff");
        voiceOff = true;
      } else if (voiceSel) {
        voiceText = t("voicePrefix") + " " + voiceSel.name + " · " + (voiceSel.gender === "male" ? "男" : "女") + (voiceSel.tone ? "，" + voiceSel.tone : "");
      } else if (settings.defaultVoice) {
        voiceText = t("voicePrefix") + " " + escapeHtml(settings.defaultVoice) + t("voiceDefaultSuffix");
      } else {
        voiceText = t("voicePrefix") + " " + t("voiceAutoVeo");
        voiceOff = true;
      }
      voiceNote.className = "dur-voice" + (voiceOff ? " dur-voice-off" : "");
      voiceNote.innerHTML = '<span class="dur-meta-icon">🎙</span><span>' + escapeHtml(voiceText) + "</span>";
      meta.appendChild(voiceNote);
    }
    if (charPool.length > 0) {
      const charNote = document.createElement("div");
      charNote.className = "dur-char";
      if (hits.length > 0) {
        const icons = hits.map(n => {
          const src = (settings.charMap && settings.charMap[n]) || "";
          if (src) return '<img class="dur-char-thumb" src="' + src.replace(/"/g, "&quot;") + '" alt="' + n.replace(/"/g, "&quot;") + '"/><span>' + escapeHtml(n) + "</span>";
          return '<span>' + escapeHtml(n) + "</span>";
        }).join("");
        charNote.innerHTML = '<span class="dur-meta-icon">👤</span>' + icons;
      } else {
        charNote.innerHTML = '<span class="dur-meta-icon">👤</span><span class="dur-none">—</span>';
      }
      meta.appendChild(charNote);
    }
...
// ---------------- Popup-side character/voice matching (mirrors content script) ----------------
// Duplicated (on purpose) from flow-automation.js so the popup can preview
// which character / voice each segment will use without injecting into the page.
function pNorm(s) {
...
function pCharHitInContext(p, nn, allNames) {
  if (!nn || !p.includes(nn)) return false;
  const isCJK = /[\u4e00-\u9fff]/.test(nn);
  const others = allNames.map(m => pNorm(m)).filter(m => m && m !== nn && m.includes(nn));
  let anyHit = false;
  ...
  while (idx !== -1) {
    ...
    if (!absorbed) anyHit = true;
    idx = p.indexOf(nn, idx + 1);
  }
  return anyHit;
}
function popupCharsInText(text, poolNames) {
  const p = pNorm(text);
  const allNames = (poolNames || []).map(n => (n || "").trim()).filter(Boolean);
  return allNames
    .map(n => (n || "").trim())
    .filter(n => {
      const nn = pNorm(n);
      if (!nn) return false;
      return pCharHitInContext(p, nn, allNames) || pTokensSubset(nn, p) || pTokensSubset(n, text);
    });
}
// 分段語音切換：在段落內任意位置寫 [NOVOICE]，則該段不填入 Flow 語音設定欄位，
// 讓 Veo 自動生成的角色聲音接管（男女聲依角色自動配對）。
function isVoiceDisabledForPrompt(text) {
  return /\[NOVOICE\]/i.test(text || "");
}
// 清除 [NOVOICE] 標籤，避免它被當成提示詞內容送出。
function cleanPromptText(text) {
  return (text || "").replace(/\[NOVOICE\]\s*/i, "");
}
// Voice preview: prompt-mentioned voice wins over the configured default.
function popupVoiceForPrompt(text) {
  if (typeof VOICES === "undefined") return null;
  if (isVoiceDisabledForPrompt(text)) return "__NOVOICE__";
  const p = pNorm(text);
  const allNames = VOICES.map(v => v.name);
  const mentioned = allNames.filter(n => pCharHitInContext(p, pNorm(n), allNames) || pTokensSubset(pNorm(n), p));
  if (mentioned.length > 0) {
    const v = VOICES.find(x => x.name === mentioned[0]);
    if (v) return v;
  }
  if (settings.defaultVoice) {
    const d = VOICES.find(x => x.name === settings.defaultVoice);
    if (d) return d;
  }
  return null;
}
...
async function scanFlowCharacters() {
...
// Save a scan result persistently and refresh all dependent UI.
function persistScanResult(dedup) {
  settings.charNames = dedup.map(c => c.name);
  settings.charMap = {};
  dedup.filter(c => c.src).forEach(c => { settings.charMap[c.name] = c.src; });
  saveSettings();
  updatePerPromptDurList();
  updateCharScanState();
}
...
function updateCharScanState() {
  ...
  if (settings.charEnabled) {
    ...
    if (settings.charNames && settings.charNames.length > 0) {
      // 自動匹配已抓到角色 → 顯示總數與各段命中角色（含縮圖）
      const countSpan = document.createElement("div");
      countSpan.className = "auto-match-summary";
      countSpan.textContent = t("scanAutoMatchedCount", settings.charNames.length);
      hint.appendChild(countSpan);
      const segList = renderAutoMatchSegments();
      if (segList) hint.appendChild(segList);
    }
...
// 自動新增角色開啟時的自動掃描：抓 Flow 頁面角色並存到 charNames/charMap，
// 再由 updateCharScanState 顯示各段命中角色。
async function doAutoCharScan() {
  if (!settings.charEnabled) return;
  // 已在掃描中則略過（避免重複注入）
  if (doAutoCharScan._running) return;
  doAutoCharScan._running = true;
  try {
    const dedup = await scanFlowCharacters();
    if (dedup === null) {
      // 不在 Flow 頁面或掃描失敗 → 提示用戶後切換回手動模式外觀
      toast(t("toastNotFlowAutoChar"));
      settings.charEnabled = false;
      saveSettings();
      loadSettings();
      renderAll();
      return;
    ...
    updatePerPromptDurList();
    updateCharScanState();
    if (dedup.length > 0) toast(t("scanFound", dedup.length));
  } catch (err) {
    toast(t("toastScanFail", err.message));
  } finally {
    doAutoCharScan._running = false;
  }
}
...
function renderAutoMatchSegments() {
  const promptsEl = document.getElementById("prompts");
  const raw = promptsEl ? promptsEl.value : "";
  const prompts = raw.split(/\n\s*\n/).map(s => s.trim()).filter(s => s.length > 0);
  const pool = settings.charNames && settings.charNames.length > 0 ? settings.charNames : [];
  if (pool.length === 0 || prompts.length === 0) return null;
  const wrap = document.createElement("div");
  wrap.className = "auto-match-segs";
  prompts.forEach((text, i) => {
    const hits = popupCharsInText(text, pool);
    if (hits.length === 0) return;
    // 第一行：抓到的角色（圖示 + 名稱，跳行顯示）
    const row = document.createElement("div");
    row.className = "auto-match-row";
    const idx = document.createElement("span");
    idx.className = "auto-match-idx";
    idx.textContent = "P" + (i + 1);
    row.appendChild(idx);
    hits.forEach(n => {
      const chip = document.createElement("span");
      chip.className = "auto-match-chip";
      const src = (settings.charMap && settings.charMap[n]) || "";
      if (src) {
        const img = document.createElement("img");
        img.src = src;
        img.alt = n;
        img.className = "auto-match-thumb";
    ...
    voiceRow.className = "auto-match-row";
    const vIdx = document.createElement("span");
    vIdx.className = "auto-match-idx";
    vIdx.textContent = "P" + (i + 1);
    voiceRow.appendChild(vIdx);
    const voiceObj = (popupVoiceForPrompt && typeof popupVoiceForPrompt === "function") ? popupVoiceForPrompt(text) : null;
    let vName = "";
    let voiceOff = false;
    if (voiceObj === "__NOVOICE__") {
      vName = t("voicePrefix") + " " + t("voiceSegOff");
      voiceOff = true;
    } else if (voiceObj && voiceObj.name) {
      vName = t("voicePrefix") + " " + voiceObj.name;
    } else if (settings.defaultVoice) {
      vName = t("voicePrefix") + " " + escapeHtml(settings.defaultVoice) + t("voiceDefaultSuffix");
    } else {
      vName = t("voicePrefix") + " " + t("voiceAutoVeo");
      voiceOff = true;
    }
    const vLabel = document.createElement("span");
    vLabel.className = "auto-match-voice" + (voiceOff ? " auto-match-voice-off" : "");
